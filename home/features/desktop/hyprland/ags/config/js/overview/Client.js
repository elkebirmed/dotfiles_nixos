...
const icon = (str) => {
  ...
  const icon = substitute(icons, str);
  const iconLow = substitute(icons, str.toLowerCase());
  const iconSym = substitute(icons, str) + "-symbolic";

  const hasIconSym = Utils.lookUpIcon(iconSym);
  const hasIcon = Utils.lookUpIcon(icon);
  const hasIconLow = Utils.lookUpIcon(iconLow);

  if (hasIconSym) return iconSym;

  if (hasIcon) return icon;

  if (hasIconLow) return iconLow;
};
